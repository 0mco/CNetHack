...
invault()
{
    ...
    fnd:
        ...
        if (u.uswallow) {
            ...
            if (!Deaf) verbalize("這裡現在怎麼樣?");
            // <<< CN_TS
            if (gsensed)
                // >>> CN_TS
                /* pline_The("other presence vanishes."); */
                pline_The("其他人的存在感消失了.");
                // <<< CN_TS
            mongone(guard);
            return;
        }
        if (youmonst.m_ap_type == M_AP_OBJECT || u.uundetected) {
            if (youmonst.m_ap_type == M_AP_OBJECT
                ...
                if (!Deaf) verbalize("嘿! 誰把%s丟在這的?",
                                    mimic_obj_name(&youmonst));
                // <<< CN_TS
            /* You're mimicking some object or you're hidden. */
            // >>> CN_TS
            /* pline("Puzzled, %s turns around and leaves.", mhe(guard)); */
            pline("%s困惑地轉身離開了.", mhe(guard));
            // <<< CN_TS
            mongone(guard);
            return;
        }
        if (Strangled || is_silent(youmonst.data) || multi < 0) {
            ...
            if (Deaf)
                ...
                pline("%s憤怒地轉身離開了.", noit_Monnam(guard));
                // <<< CN_TS
            else
                // >>> CN_TS
                /* verbalize("I'll be back when you're ready to speak to me!"); */
                verbalize("當你能夠跟我說話的時候我會回來的!");
                // <<< CN_TS
            mongone(guard);
            return;
        }
        ...
        if (Deaf)
            ...
            /*         (Blind) ? "" : "appear to "); */
            pline("%s%s並沒有認出你.", noit_Monnam(guard),
                    (Blind) ? "" : "看起來");
            // <<< CN_TS
        else
            // >>> CN_TS
        ...
        if (!umoney && !hidden_gold()) {
            if (Deaf)
                ...
                /*       (Blind) ? "" : " and beckons"); */
                pline("%s憤怒地跺腳.", noit_Monnam(guard),
                      (Blind) ? "" : "並招手示意你過去");
                // <<< CN_TS
            else
                // >>> CN_TS
            ...
            if (!umoney) {
                if (Deaf) {
                    if (!Blind)
                        ...
                        /*       invent ? "r stuff" : ""); */
                        pline("%s怒視著你%s.", noit_Monnam(guard),
                              invent ? "的東西" : "");
                        // <<< CN_TS
                } else {
                    // >>> CN_TS
...
struct monst *grd;
{
    ...
    if (movedgold || fixed) {
        if (in_fcorridor(grd, grd->mx, grd->my) || cansee(grd->mx, grd->my))
            ...
            pline("%s低聲念出一段咒語.", noit_Monnam(grd));
            // <<< CN_TS
        else
            // >>> CN_TS
...
register struct monst *grd;
{
    ...
    if (egrd->witness) {
        if (!Deaf)
            ...
            verbalize("惡棍! 你竟敢%s那金幣!",
                      (egrd->witness & GD_EATGOLD) ? "吃掉" : "毀壞");
            // <<< CN_TS
        egrd->witness = 0;
        grd->mpeaceful = 0;
        return -1;
            ...
            letknow:
                if (!cansee(grd->mx, grd->my) || !mon_visible(grd))
                    ...
                    You(um_dist(grd->mx, grd->my, 2)
                            ...
                            : "面對著%s.",
                        /* "an angry guard" */
                        x_monnam(grd, ARTICLE_A, "憤怒的", 0, FALSE));
                    // <<< CN_TS
                return -1;
            } else {
                if (!Deaf)
    ...
    if (egrd->fcend > 1) {
        if (egrd->fcend > 2 && in_fcorridor(grd, grd->mx, grd->my)
            ...
            pline("%s感到困惑, 然後走了.", noit_Monnam(grd));
            // <<< CN_TS
...
paygd()
{
    ...
    if (u.uinvault) {
        ...
        Your("%ld %s歸入了{{Magic Memory Vault}}.", umoney,
            // <<< CN_TS
             currency(umoney));
        gx = u.ux;
        gy = u.uy;
