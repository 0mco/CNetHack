...
int *dmg_p; /* for dishing out extra damage in lieu of Int loss */
{
    ...
    if (flesh_petrifies(pd)) {
        ...
        if (magr == &youmonst) {
            if (!Stone_resistance && !Stoned)
                ...
                make_stoned(5L, (char *) 0, KILLED_BY_AN, pd->cname);
                // <<< CN_TS
        } else {
    ...
    if (magr == &youmonst) {
        ...
        if (mindless(pd)) { /* (cannibalism not possible here) */
            ...
            Sprintf(killer.name, "unwisely ate the brain of %s", pd->cname);
            // <<< CN_TS
...
register int pm;
{
    ...
    if (flesh_petrifies(&mons[pm])) {
        if (!Stone_resistance
            ...
            Sprintf(killer.name, "tasting %s meat", mons[pm].cname);
            // <<< CN_TS
            killer.format = KILLED_BY;
            You("turn to stone.");
            done(STONING);
    ...
    case PM_LARGE_CAT:
        ...
        if (!CANNIBAL_ALLOWED()) {
            ...
            You_feel("that eating the %s was a bad idea.", mons[pm].cname);
            // <<< CN_TS
            HAggravate_monster |= FROMOUTSIDE;
        }
        break;
    ...
    case PM_FAMINE: {
        ...
        Sprintf(killer.name, "unwisely ate the body of %s", mons[pm].cname);
        // <<< CN_TS
        killer.format = NO_KILLER_PREFIX;
        done(DIED);
        /* life-saving needed to reach here */
    ...
    case PM_SMALL_MIMIC:
        ...
        if (youmonst.data->mlet != S_MIMIC && !Unchanging) {
            ...
            Sprintf(buf,
                    ...
                    an(Upolyd ? youmonst.data->cname : urace.noun));
                    // <<< CN_TS
            eatmbuf = dupstr(buf);
            nomovemsg = eatmbuf;
            afternmv = eatmdone;
    ...
    if (obj && buf) {
        if (r == SPINACH_TIN)
            ...
            if (vegetarian(&mons[mnum]))
                ...
                Sprintf(eos(buf), "%s", mons[mnum].cname);
                // <<< CN_TS
            else
                // >>> CN_TS
                /* Sprintf(eos(buf), "%s meat", mons[mnum].mname); */
                Sprintf(eos(buf), "%s meat", mons[mnum].cname);
                // <<< CN_TS
        }
    }
}
...
const char *mesg;
{
    ...
    if (r != SPINACH_TIN) {
        ...
        You("consume %s %s.", tintxts[r].txt, mons[mnum].cname);
        // <<< CN_TS
